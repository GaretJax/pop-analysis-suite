...
DESCRIPTION: implementation of the Job Manager service
...
JobMgr::JobMgr(bool daemon, const paroc_string &challenge, const paroc_string &url, const paroc_accesspoint &nodeAccess): JobCoreService(challenge)
{
	//Added by clementval
	
	//Save the POPCSearchNode's access point for future communication
	localNode = nodeAccess;
	//Create interface to communicate with the local POPCSearchNode
	POPCSearchNode myNode(localNode);
	//Set a reference of this JobMgr to the POPCSearchNode
	myNode.setJobMgrAccessPoint(GetAccessPoint());
	...
	*tmp=0;
	...
	paroc_timerthread *timeinfo=new paroc_timerthread(SELFREGISTER_TIMEOUT,0, this);
	timeinfo->create();
...
JobMgr::JobMgr(bool daemon, const paroc_string &conf, const paroc_string &challenge, const paroc_string &url, const paroc_accesspoint &nodeAccess): JobCoreService(challenge)
{
	//Added by clementval
...
//  assert(fd0==0 && fd1==1 && fd2==2);
	...
	myNode.setPOPCSearchNodeId(localNode.GetAccessString());
	DEBUG("Node ID : %s", myNode.getPOPCSearchNodeId().GetString());

	//Setting the current operating system (The one of the JobMgr's machine)
	myNode.setOperatingSystem(paroc_system::platform);
	DEBUG("Node opertating system : %s", myNode.getOperatingSystem().GetString());

	//Setting the total computing power of this JobMgr
	myNode.setPower(total.flops);
	DEBUG("Node power : %f", myNode.getPower());

	//Setting the total RAM of this JobMgr
	myNode.setMemorySize(total.mem);
	DEBUG("Node memory size : %d", myNode.getMemorySize());

	//Setting the total network bandwith of this JobMgr
	myNode.setNetworkBandwidth(total.bandwidth);
	DEBUG("Node bandwidth : %d", myNode.getNetworkBandwidth());
...
void JobMgr::UnregisterNode(const paroc_accesspoint &url){
	...
	myNode.removeNeighbor(remoteNode);
}
//End of add
...
int JobMgr::CreateObject(const paroc_accesspoint &localservice, const paroc_string &objname, const paroc_od &od, int howmany, paroc_accesspoint* objcontacts)
{
	...
	try
	{
		...
		while (count<howmany && retry>0)
		{
			...
			for (int i=count;i<howmany;i++) if (!jobcontacts[i].IsEmpty())
				
				{
					...
					{
						paroc_service_log("Exception catch in ExecObj");
					}
				}
...
//	if(!(getenv("POPC_SEARCHNODETEST")) && !od.isSearchSet()){
	...
	r.setNodeId(myNode.getPOPCSearchNodeId());
	...
	POPCSearchNodeInfos responses = myNode.launchDiscovery(r, timeoutint);
	...
	for(jobindex = 0; jobindex < howmany; jobindex++){
		...
		paroc_accesspoint jm_ap = n.getJobMgrAccessPoint();
		//Contact the JobMgr
		JobMgr jobmgr(jm_ap);	
		float t;
		//Try to reserve on remote JobMgr 
		reserveIDs[jobindex] = jobmgr.Reserve(od, t);	
		...
		if(reserveIDs[jobindex] == 0){
			jobindex--;
			popc_node_log("UNABLE TO RESERVE ON %s", jm_ap.GetAccessString());
			failedReservation++;
			...
			popc_node_log("RESID;%d;NODEID;%s", reserveIDs[jobindex], jm_ap.GetAccessString());
			//Setting the fitness
			fitness[jobindex] = t;
		}
		ni++;
...
// This method is quite the same as MatchAndReserve but was necessary to reserve the resource before execution
// The matching is done in the resource discovery
int JobMgr::Reserve(const paroc_od &od, float &inoutfitness){
...
/*
	bool test = false;
	test=getenv("POPC_SEARCHNODETEST");
*/
	//End
	...
	mutex
	{
		...
		//&& !test should be removed in production release
//		if (!od.IsEmpty() && !test)
		if (!od.IsEmpty() )
		{
		...
		popc_node_log("GIVE_RESID;%d", t.Id);
		return t.Id;
	}
}
//End add
...
{
	if (howmany<=0) {
		popc_node_log("Exec failed for howmany");
		return EINVAL;
	}
	...
	mutex {
		for (int i=0;i<howmany;i++)
		{
			...
			if (r==NULL)
			{
				...
				popc_node_log("Exec failed for reservation");
				return ECANCELED;
			}
	...
	*curenv=NULL;
	...
	try
	{
		...
		{
			CancelReservation(reserveIDs,howmany);
			popc_node_log("Exec failed for codemgr");
			return ENOENT;
		}
	}
	catch (...)
	{
		CancelReservation(reserveIDs, howmany);
		popc_node_log("Exec failed for codemgr 2");
		return ENOENT;
	}
	...
	{
		CancelReservation(reserveIDs,howmany);
		popc_node_log("Exec fialed for socket");
		return errno;
	}
	...
	if (ret!=0)
	{
		Pause(localservice, SLEEP_TIME_ON_ERROR);
		CancelReservation(reserveIDs, howmany);
		popc_node_log("Exec failed for return Exec");
		return ret;
	}
	...
	for (int i=0;i<howmany;i++)
	{
		...
		if (!buf.Recv(tmpsock))
		{
			...
			Pause(localservice, SLEEP_TIME_ON_ERROR);
			CancelReservation(reserveIDs, howmany);
			popc_node_log("Exec failed for accesspoint");
			return err;
		}
