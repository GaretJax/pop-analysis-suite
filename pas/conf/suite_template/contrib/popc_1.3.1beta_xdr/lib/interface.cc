	...
	else if (!TryLocal(objaccess))
	{
		...
		try
		{
			...
			if (batchindex==0 && batchsize>1)
			{
				...
				ret=resources.CreateObject(paroc_system::appservice,objname,od, batchsize, batchaccesspoint);
				if (ret==0) objaccess=batchaccesspoint[batchindex++];
			}
			else{
				DEBUG("Create Object : %s\n", ClassName());
				ret=resources.CreateObject(paroc_system::appservice,objname,od, 1, &objaccess);
			}
...
void paroc_interface::NegotiateEncoding(paroc_string &enclist, paroc_string &peerplatform)
{
	...
	if (enc_pref.IsEmpty())
	{
		...
		while (pos!=NULL)
		{
			...
			if (paroc_utils::MatchWildcard(enc,"raw*")) continue;
			// end of replacement
