...
def host_option(parser):
    """
    Adds an optional --hosts option to the given parser which always returns a
    list of hosts specified on the command line.
    
    Multiple hosts can be separated with a comma.
    """
    validate = lambda h: h.split(',') if h else []
    parser.add_argument('--hosts', type=validate, default='')
...
def build_parser():
    ...
    for cmd in os.listdir(os.path.dirname(__file__)):
        ...
        if cmd.endswith('.py') and not cmd.startswith('_'):
            ...
            # ...and build the suparser...
            subparser = subparsers.add_parser(name, help=getdoc(module))
            subparser.set_defaults(execute=module.command)
            
            # ...by letting the module customize it, if it needs to.
